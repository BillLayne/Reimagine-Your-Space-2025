...
// Edge Functions use Deno Context
export default async (request: Request, context: any) => {
    ...
    try {
        ...
        const API_KEY = Deno.env.get('API_KEY') || Deno.env.get('GEMINI_API_KEY');
        ...
        switch (action) {
            ...
            case 'integrateFurniture': {
                ...
                const parts = [
                    ...
                    ...furnitureImages.map((img: any) => ({ inlineData: { data: img.data, mimeType: img.mimeType } })),
                    { text: metaPrompt }
                ];
                const response = await ai.models.generateContent({ model: 'gemini-2.5-flash-image', contents: { parts }, config: { responseModalities: [Modality.IMAGE, Modality.TEXT] } });
                const imagePart = response.candidates?.[0]?.content?.parts.find((p: any) => p.inlineData);
